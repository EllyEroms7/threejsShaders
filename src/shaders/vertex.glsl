...
return (maxAllowed - minAllowed) * (unscaled - originalMin) / (originalMax - originalMin);
}

float wave(vec3 position) {
return fit(smoothMod(position.y * 7.0, 1.1, 1.0), .35, .6, .0, 1.0);
}
...
vec3 coords = normal;
coords += uTime / 8.0;
vec3 noisePatterns = vec3(noise(coords));
float pattern = wave(noisePatterns);
...
vec4 pos = modelViewProjectionMatrix * vec4(newPosition, 1.0); //mvp
gl_Position = pos;
}`